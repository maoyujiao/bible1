...
import com.iyuba.core.me.protocol.RequestUserDetailInfo;
import com.iyuba.core.me.protocol.ResponseUserDetailInfo;
...
public class EditUserInfoActivity extends BasisActivity {
    ...
    private void initWidget() {
        ...
        tvUserName = findView(R.id.tourName);
        edUserName = findView(R.id.editName);

        if (TouristUtil.isTourist()) {
            tvUserName.setText(AccountManager.Instace(mContext).getId());
            edUserName.setVisibility(View.VISIBLE);
            edUserName.setVisibility(View.GONE);
        } else {
            edUserName.setText(AccountManager.Instace(mContext).getUserName());
            edUserName.setVisibility(View.VISIBLE);
        }
    ...
    private void editUserName() {
        ...
        if (name.equals(AccountManager.Instace(mContext).getUserName())) {
            return;
        }

        String sign = 10012 + AccountManager.Instace(mContext).getId() + "iyubaV2";
        String url = "";
        try {
            url = "http://api.iyuba.com.cn/v2/api.iyuba?format=json&protocol=" +
                    "10012" +
                    "&uid=" + AccountManager.Instace(mContext).getId() +
                    "&oldUsername=" + URLEncoder.encode(AccountManager.Instace(mContext).getUserName(), "UTF-8") +
                    "&username=" +
        ...
        Http.get(url, new HttpCallback() {
            ...
            public void onSucceed(Call call, String response) {
                ...
                try {
                    ...
                    String result = jsonObjectRoot.getString("result");
                    if (result.equals("121")) {
                        showShort("昵称修改成功！");
                        AccountManager.Instace(mContext).userName = name;
                        UserInfo userInfo = AccountManager.Instace(mContext).userInfo;
                        userInfo.username = name;
                        AccountManager.Instace(mContext).userInfo = userInfo;
                        ConfigManager.Instance().putString("userName", name);
